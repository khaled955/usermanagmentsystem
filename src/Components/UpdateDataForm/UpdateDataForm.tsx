...
import { FormValues, UserDataDecoded } from "../../Interfaces/interfaces"
...
export default function UpdatDataForm() {
...
const validationSchema =Yup.object().shape({
 ...
 const {handleUpdateHideForm} = modalContextControl
...
const userInfoDecoded: UserDataDecoded | null = token ? jwtDecode(token) : null;
  ...
  async function handleSubmitData(data:FormValues){
    let id;
    if(localStorage.getItem("currentIdUpdate")){
        id = localStorage.getItem("currentIdUpdate")
    }
    const options ={
...
try {
  ...
  toast.success("Updated Your Data Done Successfully")
  setTimeout(()=>{
    handleUpdateHideForm()
...
transition={{duration:1}}
  ...
  <i onClick={handleUpdateHideForm} className="fa-solid fa-rectangle-xmark absolute top-2 right-2 cursor-pointer text-3xl"></i>
  <div className="login-box">
      ...
      <h1 className="font-black text-xl font-serif mb-5">User Update Data Form</h1>
      </div>
    </div>
    <div className="form">
    ...
    <button className="my-btn" type="submit"> Update</button>
      </form>
    </div>
